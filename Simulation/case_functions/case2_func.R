# Function for case 2. Please read the walk through for an explanation of how this works.
library(tidyverse)
library(nlme)
library(MASS)

sims_function_case2 <- function(N, lag_amount){
    ...
    for(i in 1:N){ 
      ...
      for(j in 1:time){ 
        ...
        if(j == 1 | j == 2 | j == 3 | j == 4 | j == 5){
          ...
          df[count,1] = j
          df[count,2] = i
          # y generated first in this case because it is causal ---------------------
          df[count,4] = autor_y * df[(count - 1),4] + y_het + rnorm(1,0,1)
          df[count,3] = autor_x * df[(count - 1),3] + y_cause_x * df[count - lag_amount,4] + rnorm(1,0,1) # lag amount parameter plugged in here

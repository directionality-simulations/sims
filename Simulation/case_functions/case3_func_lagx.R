...
sims_function_case3_lagx <- function(N, lag_amount){
    ...
    for(i in 1:N){ 
      ...
      for(j in 1:time){ 
        ...
        if(j == 1 | j == 2 | j == 3 | j == 4 | j == 5){
          ...
          df[count,3] = autor_x * df[(count - 1),3] + z_cause * df[(count - lag_amount),5] + rnorm(1,0,1) # lag parameter here
          df[count,4] = autor_y * df[(count - 1),4] + z_cause * df[count,5] + y_het + rnorm(1,0,1) 
